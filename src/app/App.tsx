...
function Overlay() {
  ...
  useEffect(()=>{gameAudio.setEnabled(game.soundEnabled);gameAudio.scene(scene.id)},[game.soundEnabled,scene.id])
  useEffect(()=>{if(game.dialogue)gameAudio.dialogue()},[game.dialogueIndex,game.dialogue])
  useEffect(()=>{if(game.sceneComplete)gameAudio.chapter()},[game.sceneComplete])
  return <>
    <header className="hud"><div><p>{scene.number}</p><strong>{scene.title}</strong></div><div className="story-progress"><span>{completed + 1} / {totalStoryBeats}</span><i style={{width:`${((completed+1)/totalStoryBeats)*100}%`}} /></div><div className="hud-actions"><button aria-label="Toggle sound" onClick={()=>{game.toggleSound();gameAudio.start()}}>{game.soundEnabled?'Sound on':'Sound off'}</button><button onClick={game.toggleJournal}>Journal <b>{game.journal.length}</b></button><button onClick={game.togglePause}>{game.paused&&!game.journalOpen?'Resume':'Pause'}</button></div></header>
    ...
    {game.nearby && !game.dialogue && !game.choosing && !game.puzzleActive && !game.sceneComplete && <button className="interact-prompt" onClick={game.interact}><kbd>E</kbd> {step.action}</button>}
    ...
    {game.puzzleActive&&<PuzzleOverlay key={`${scene.id}:${step.id}`} puzzle={puzzleFor(scene.id,step.id)!}/>} 
    <Controls />
