...
type GameState = {
  ...
  puzzleActive: boolean; puzzleSolvedCurrent: boolean
...
function finishStep(state: GameState) {
  ...
  if (state.stepIndex === scene.steps.length - 1) return { dialogue: undefined, dialogueIndex: 0, choosing: false, sceneComplete: true, nearby: false, journal, burden, puzzleSolvedCurrent:false, puzzleActive:false }
  return { dialogue: undefined, dialogueIndex: 0, choosing: false, stepIndex: state.stepIndex + 1, nearby: false, journal, burden, puzzleSolvedCurrent:false, puzzleActive:false }
}
...
export const useGame = create<GameState>()(persist((set, get) => ({
  ...
  nearby: false, dialogueIndex: 0, choosing: false, sceneComplete: false, gameComplete: false, journal: [], soundEnabled: true, puzzleActive:false, puzzleSolvedCurrent:false,
  start: () => set({ started: true, paused: false }),
  reset: () => set({ started: true, paused: false, journalOpen: false, sceneIndex: 0, stepIndex: 0, burden: 0, nearby: false, dialogue: undefined, dialogueIndex: 0, choosing: false, sceneComplete: false, gameComplete: false, journal: [], puzzleActive:false, puzzleSolvedCurrent:false }),
  togglePause: () => set((s) => ({ paused: !s.paused })),
  toggleJournal: () => set((s) => ({ journalOpen: !s.journalOpen, paused: !s.journalOpen })),
  setNearby: (nearby) => set({ nearby }),
  setMessage: (message) => set({ message }),
  toggleSound: () => set((s)=>({soundEnabled:!s.soundEnabled})),
  completePuzzle:()=>{const s=get();const step=storyScenes[s.sceneIndex].steps[s.stepIndex];set({puzzleActive:false,puzzleSolvedCurrent:true,dialogue:step.dialogue,dialogueIndex:0})},
  interact: () => {
    const s = get(); if (!s.nearby || s.dialogue || s.sceneComplete) return
    const step = storyScenes[s.sceneIndex].steps[s.stepIndex]
    if(puzzleFor(storyScenes[s.sceneIndex].id,step.id)&&!s.puzzleSolvedCurrent){set({puzzleActive:true});return}
  ...
  continueScene: () => {
    ...
    else set({ sceneIndex: s.sceneIndex + 1, stepIndex: 0, sceneComplete: false, nearby: false, dialogue: undefined, dialogueIndex: 0, puzzleActive:false,puzzleSolvedCurrent:false })
  },
}), {
  name: 'narrow-way-save-v2',
  partialize: (state) => ({ started: state.started, sceneIndex: state.sceneIndex, stepIndex: state.stepIndex, burden: state.burden, journal: state.journal, gameComplete: state.gameComplete, soundEnabled:state.soundEnabled }),
}))
