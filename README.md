...
Current build contains 223 gated story beats across 30 distinct painterly low-poly environments, a 42-variant animated human cast, persistent Hopeful companion, 360-degree third-person camera controls, cinematic conversations, scene-specific recorded ambience and sound cues, journal discoveries, choices, persistent story items, and 47 symbolic puzzle trials. Journey: Dream → City of Destruction → Evangelist’s Field → Slough of Despond → Worldly Wiseman and Mount Sinai → Wicket Gate → Interpreter’s House → Cross → three sleepers → Formalist and Hypocrisy → Hill Difficulty → lost roll → chained lions → Palace Beautiful → Valley of Humiliation and Apollyon → Valley of the Shadow of Death → Faithful → Talkative → Evangelist’s warning → Vanity Fair → Hopeful → By-Ends → Demas → By-Path Meadow → Doubting Castle → Delectable Mountains → Enchanted Ground → Beulah → River → Celestial City. See `GAME_STATUS.md` for progress.
